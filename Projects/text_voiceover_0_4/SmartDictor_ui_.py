...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow, voice_name: str):
        self.speakers = VoiceOptions().find_voice()
        ...
        BaseVoiceOptions(speakers=self.speakers,voice_file='voise.txt',voice_opt_file='opt.txt')
...
class thread(QThread): # создание и фукции потока
    ...
    def run(self): # запуск потока
        self.Dictor = RunApp(speakers=self.speakers, voice_file='voise.txt',voice_opt_file='opt.txt',img="tesseract_mask.png",lang='rus', pytesseract_config=r'--oem 3  --psm 4 tessedit_char_whitelist=абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ',time_replay='Время повтора', min_speed='Минималья скорость', max_speed='Максимальная скорость',min_volume='Минимальная громкость', max_volume='Максимальная громкость') # паралельная функция
...
if __name__ == "__main__":
    ...
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow,  voice_name='voise.txt')
    MainWindow.show()
    sys.exit(app.exec_())
